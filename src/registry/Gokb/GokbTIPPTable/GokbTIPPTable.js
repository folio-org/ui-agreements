...
const GokbTIPPTable = ({ resourceId }) => {
  ...
  const packageIdentifiersQuery = pkgIdentifiers
    .map(
      (pid) => `(identifiers.identifier.ns.value==gokb_uuid&&identifiers.identifier.value==${pid}&&identifiers.status.value==approved)`
    )
    .join('||');
  ...
  const pciChunkedQueryIdTransform = useCallback(
    (chunkedIds) => {
          ...
          filters: [
            {
              value: chunkedIds
                .map((titleId) => {
                  const relevantTipp = tipps.find(
                    (t) => t.tippTitleUuid === titleId
                  );
  ...
  const { items: pcis, isLoading: arePcisLoading } = useChunkedIdTransformFetch(
    {
      ...
      reduceFunction: (queries) => queries.reduce((acc, curr) => {
  ...
  const getPackageInKb = (pkgUuid) => {
    const filteredPackages = packages.filter((p) => p.identifiers.find(
      (io) => io.status?.value === 'approved' &&
  ...
  const getPCIInKB = (titleUuid, packageUuid) => {
    const filteredPCIs = pcis.filter(
      (pci) => pci.pti.titleInstance.identifiers.find(
        (io) => io.status?.value === 'approved' &&
            io.identifier.ns.value === 'gokb_uuid' &&
            io.identifier.value === titleUuid
      ) &&
        pci.pkg.identifiers.find(
          (io) => io.status?.value === 'approved' &&
            io.identifier.ns.value === 'gokb_uuid' &&
  ...
  const formatter = {
    coverage: (rowData) => {
      const pciInKB = getPCIInKB(
        rowData.tippTitleUuid,
        rowData.tippPackageUuid
      );
      return <Coverage pci={pciInKB ?? rowData} />; // GOKB Coverage shape happens to be similar to ours, but this feels flaky.
    },
    pkgName: (rowData) => {
    ...
    titleInKB: (rowData) => {
      ...
      if (!titleInKB) {
        return (
          <FormattedMessage id="ui-agreements.gokbSearch.basket.titleNotInKB" />
        );
      }

      return (
        <Link to={`/erm/titles/${titleInKB.id}`}>
          <FormattedMessage id="ui-agreements.gokbSearch.basket.viewInKB" />
        </Link>
      );
    },
    actions: (rowData) => {
  ...
  return (
    <MultiColumnList
      columnMapping={{
        pkgName: (
          <FormattedMessage id="ui-agreements.gokbSearch.basket.pkgName" />
        ),
        coverage: <FormattedMessage id="ui-agreements.eresources.coverage" />,
        platform: <FormattedMessage id="ui-agreements.eresources.platform" />,
        syncStatus: (
          <FormattedMessage id="ui-agreements.gokbSearch.basket.syncStatus" />
        ),
        titleInKB: (
          <FormattedMessage id="ui-agreements.gokbSearch.basket.titleInKB" />
        ),
        actions: <FormattedMessage id="ui-agreements.actions" />,
      }}
      columnWidths={{
        // Enforce certain widths
        coverage: 250,
        actions: 75,
      }}
      contentData={tipps}
