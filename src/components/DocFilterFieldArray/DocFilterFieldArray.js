...
const DocFilterFieldArray = ({ atTypeValues }) => {
  const {
    mutators: { push },
  } = useForm();
  ...
  return (
    <>
      ...
      <Button onClick={() => push('filters', { rules: [{}] })}>
        <FormattedMessage id="ui-agreements.documentFilter.addDocumentFilter" />
      </Button>
    </>
  );
};

DocFilterFieldArray.propTypes = propTypes;
export default DocFilterFieldArray;
