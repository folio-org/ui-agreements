...
describe('TitleCardExternal', () => {
  describe('with title resource', () => {
    ...
    /* test('renders a link with the title name', () => {
      ...
      expect(getByRole('link', { name: 'The Astronomical Journal' })).toBeInTheDocument();
    }); */
  ...
  describe('with titleReferenceObject resource', () => {
    ...
    /* test('renders a link with the title name', () => {
      const { getByRole } = renderComponent;
      expect(getByRole('link', { name: 'The Doppler Method for the Detection of Exoplanets', hidden: true })).toBeInTheDocument();
    }); */
