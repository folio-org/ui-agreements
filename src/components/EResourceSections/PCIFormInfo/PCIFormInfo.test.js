...
describe('PCIFormInfo', () => {
  ...
  test('renders expected value in the Name TextField', async () => {
    await TextField({ id: 'edit-pci-name' }).has({ value: initialValues.name });
  });

  test('renders the Suppress from discovery Checkbox', async () => {
    await Checkbox({ id: 'pci-suppress-from-discovery' }).exists();
  });

  // Add Datepicker tests for accessiblefrom and accessibleUntil fields once the interactor is available
});
