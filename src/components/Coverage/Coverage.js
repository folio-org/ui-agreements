...
export default class Coverage extends React.Component {
  ...
  */
  ...
  render() {
    ...
    if (isLine) {
      if ('resource' in pci && pci?.resource?._object?.pti?.titleInstance?.type?.value === resourceTypes.MONOGRAPH) {
        isMonograph = true;
        pciToRender = pci?.resource?._object;
      } else if (pci?._object?.pti?.titleInstance?.type?.value === resourceTypes.MONOGRAPH) {
        isMonograph = true;
        pciToRender = pci?._object;
      }
    } else if (pci?.pti?.titleInstance?.type?.value === resourceTypes.MONOGRAPH) {
      isMonograph = true;
    }
