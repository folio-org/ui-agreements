...
describe('Coverage', () => {
  ...
  test('renders expected coverage start volume in the row', async () => {
    Promise.all([
      ...
      await MultiColumnListCell({ row: 1, columnIndex: 1 }).has({ content: '10/04/2021' })
    ]);
  });
  ...
  test('renders expected coverage start issue in the row', async () => {
    Promise.all([
      ...
      await MultiColumnListCell({ row: 1, columnIndex: 2 }).has({ content: '10/04/2021' })
    ]);
  });
  ...
  test('renders expected coverage end volume in the row', async () => {
    Promise.all([
      ...
      await MultiColumnListCell({ row: 1, columnIndex: 4 }).has({ content: '10/28/2021' })
    ]);
  });
  ...
  test('renders expected coverage end issue in the row', async () => {
    Promise.all([
      ...
      await MultiColumnListCell({ row: 1, columnIndex: 5 }).has({ content: '10/28/2021' })
    ]);
  });
