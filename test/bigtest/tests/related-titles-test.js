...
describe.only('RelatedTitleInfo', () => {
  ...
  describe('related monograph titles', () => {
    ...
    relatedTitlesMonograph.forEach((monographResource, i) => {
      ...
      it(`should render expected title with index ${i}`, () => {
        expect(interactor.titleInstanceName).to.equal('Related title "' + monographResource.name + '"');
      });

      it('should render expected subType', () => {
        expect(interactor.titleInstanceSubType).to.equal(monographResource.subType.label);
      });

      if (isbnValue) {
        it('should render expected ISBN', () => {
          expect(interactor.titleInstanceISBN).to.have.string(isbnValue);
        });
      }

      if (doiValue) {
        it('should render expected DOI', () => {
          expect(interactor.titleInstanceDOI).to.have.string(doiValue);
        });
      }
    });
  });
  ...
  describe('related serial titles', () => {
    ...
    relatedTitlesSerial.forEach((serialResource, i) => {
      ...
      it(`should render expected title with index ${i}`, () => {
        expect(interactor.titleInstanceName).to.equal('Related title "' + serialResource.name + '"');
      });

      it('should render expected subType', () => {
        expect(interactor.titleInstanceSubType).to.equal(serialResource.subType.label);
      });

      if (ezbValue) {
        it('should render expected EZB', () => {
          expect(interactor.titleInstanceEZB).to.have.string(ezbValue);
        });
      }

      if (zdbValue) {
        it('should render expected ZDB', () => {
          expect(interactor.titleInstanceZDB).to.have.string(zdbValue);
        });
      }

      if (eissnValue) {
        it('should render expected ISSN (Online)', () => {
          expect(interactor.titleInstanceEISSN).to.have.string(eissnValue);
        });
      }

      if (issnValue) {
        it('should render expected ISSN (Print)', () => {
          expect(interactor.titleInstanceISSN).to.have.string(issnValue);
        });
      }
    });
  });
